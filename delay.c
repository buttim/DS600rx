...
void Timer3_Delay100ms(uint32_t u32CNT) {
  ...
  while (u32CNT != 0) {
    RL3 = LOBYTE(TIMER_DIV128_VALUE_100ms); // Find  define in
                                            // "Function_define.h" "TIMER VALUE"
...
/*void Timer2_Delay500us(uint32_t u32CNT) {
  ...
  while (u32CNT != 0) {
    TL2 = LOBYTE(TIMER_DIV4_VALUE_500us); // Find  define in "Function_define.h"
                                          // "TIMER VALUE"
...
void Timer3_Delay10us(uint32_t u32CNT) {
  ...
  set_TR3;      // Trigger Timer3
  while (u32CNT != 0) {
    RL3 = LOBYTE(TIMER_DIV4_VALUE_10us); // Find  define in "Function_define.h"
                                         // "TIMER VALUE"
